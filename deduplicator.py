...
class Processor:
    def __init__(self, filename, logfile="tests/log.txt"):
        ...
        self.logger.print(f"Starting Process on <{filename}>. Initial Data: {self.input_data}")
    ...
    def remove_by_id(self, cur_id):
        if cur_id in self.unique_ids:
            ...
            self.logger.print(
                f"Element {self.unique_ids[cur_id]} removed: ID Conflict on {cur_id}"
            )
    ...
    def remove_by_email(self, cur_email):
        if cur_email in self.unique_emails:
            ...
            self.logger.print(
                f"Element {self.unique_emails[cur_email]} removed: Email Conflict on {cur_email}"
            )
    ...
    def process_json(self):
        ...
        for i in range(self.num_elements):
            ...
            self.logger.print(f"Processing Element {i} | {self.input_data[i]}")
    ...
    def generate_file(self, out_file="out.json"):
        ...
        self.logger.print(
            f"Removed Elements: {[val for val in self.input_data if val not in output]}"
        )
        self.logger.print(f"Final Data: {output}")
        self.logger.print(f"Starting Write to File <{out_file}>")
